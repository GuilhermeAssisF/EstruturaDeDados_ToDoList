...
[Symbol.iterator]() {         
  ...
  reordenarPorPrioridade() {
    // Se a lista estiver vazia ou tiver apenas um item, não há necessidade de reordenar.
    if (this.length <= 1) return;

    // Converta a lista ligada para um array para facilitar a ordenação.
    const arrayTarefas = [];
    let currentNode = this.head;
    while (currentNode !== null) {
        arrayTarefas.push(currentNode.dado);
        currentNode = currentNode.prox;
    }

    // Ordenar o array de tarefas pela prioridade (assumindo que menor valor de prioridade é mais alta).
    arrayTarefas.sort((a, b) => a.prioridade - b.prioridade);

    // Reconstrua a lista ligada usando o array ordenado.
    this.head = null;
    this.tail = null;
    this.length = 0;
